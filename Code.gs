...
var calendars = [
  {
    source: "icsURL1",
    target: "targetCalendar1",
    onlyFutureEvents: true,
  },
];

const timeout = 36000; // Fixed timeout for running the script
const pastDaysToSync = 1; // Sync only these many days of even around the current date
const futureDaysToSync = 5; // Sync only these many days of even around the current date
...
var defaults = {
  onlyFutureEvents: false, // If you turn this to "true", past events will not be synced (this will also removed past events from the target calendar if removeEventsFromCalendar is true)
  addEventsToCalendar: true, // If you turn this to "false", you can check the log (View > Logs) to make sure your events are being read correctly before turning this on
  modifyExistingEvents: true, // If you turn this to "false", any event in the feed that was modified after being added to the calendar will not update
  removeEventsFromCalendar: true, // If you turn this to "true", any event created by the script that is not found in the feed will be removed.
  addTasks: false,
};
